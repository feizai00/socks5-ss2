...
generate_config() {
    ...
    socks_user=$(escape_json "$socks_user")
    socks_pass=$(escape_json "$socks_pass")
...
main_menu() {
    while true; do
        ...
        case $choice in
            ...
            4) renew_service; read -p "按回车继续..." ;;
            5) delete_service; read -p "按回车继续..." ;;
            6) delete_all_services; read -p "按回车继续..." ;;
...
delete_all_services() {
    ...
    for port_dir in "$SERVICE_DIR"/*; do
        if [ -d "$port_dir" ]; then
            ...
            local info_file="$port_dir/info"
            if [ -f "$info_file" ]; then
                node_name=$(grep "NODE_NAME=" "$info_file" 2>/dev/null | cut -d'=' -f2- || echo "未知")
            fi
            
            # 获取状态
            local status="未知"
            if [ -f "$port_dir/xray.pid" ]; then
                local pid=$(cat "$port_dir/xray.pid")
                if kill -0 "$pid" 2>/dev/null; then
                    status="运行中"
                else
                    status="已停止"
    ...
    # 三重确认
    echo -e "${YELLOW}⚠️  警告：此操作将删除所有服务，包括配置文件和日志！${NC}"
    echo ""
    
    # 第一次确认
    echo -n "请输入 'DELETE' 确认删除所有服务: "
    read confirm1
    ...
    # 第三次确认（数字确认）
    echo ""
    echo -n "最后确认，请输入服务数量 $count: "
    read confirm3
    if [ "$confirm3" != "$count" ]; then
        log_warning "操作已取消"
        return 0
    fi
    ...
    # 创建自动备份
    echo "正在创建删除前备份..."
    local backup_name="delete_all_backup_$(date +%Y%m%d_%H%M%S).tar.gz"
    local backup_path="$CONFIG_DIR/$backup_name"
    
    if tar -czf "$backup_path" -C "$CONFIG_DIR" services/ 2>/dev/null; then
        log_success "已创建备份: $backup_name"
    else
        log_warning "备份创建失败，但将继续删除操作"
    fi
    ...
    for port_dir in "$SERVICE_DIR"/*; do
        if [ -d "$port_dir" ]; then
            ...
            # 删除目录
            if rm -rf "$port_dir" 2>/dev/null; then
                echo -e "${GREEN}✅${NC}"
                deleted=$((deleted + 1))
            else
                echo -e "${RED}❌${NC}"
    ...
    if [ $failed -eq 0 ]; then
        ...
        echo "📍 如需恢复，请使用菜单中的 '恢复配置' 功能"
    else
        log_error "删除完成，但有 $failed 个服务删除失败"
        echo "建议手动检查 $SERVICE_DIR 目录"
    fi
}
...
main() {
    ...
    if [ $# -gt 0 ]; then
        case "$1" in
            "ls"|"list")                list_services                ;;            "add")                if [ $# -ge 2 ]; then                    echo "添加服务: $2"                    add_service                else                    echo "用法: $0 add"                fi                ;;            "start")                if [ $# -ge 2 ]; then                    echo "启动服务: $2"                    # 这里调用已有的start_service函数                else                    echo "用法: $0 start PORT"                fi                ;;            "stop")                if [ $# -ge 2 ]; then                    echo "停止服务: $2"                    # 这里调用已有的stop_service函数                else                    echo "用法: $0 stop PORT"                fi                ;;            "help"|"--help"|"-h")                echo "Xray SOCKS5转SS 管理工具"                echo "交互模式: $0"                echo "命令行模式:"                echo "  $0 ls          - 列出服务"                echo "  $0 add         - 添加服务"                echo "  $0 start PORT  - 启动服务"                echo "  $0 stop PORT   - 停止服务"                echo "  $0 help        - 显示帮助"                ;;
            "start_single_service")
                if [ $# -ge 2 ]; then
